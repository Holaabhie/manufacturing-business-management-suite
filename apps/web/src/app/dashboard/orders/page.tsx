...
export default function OrdersPage() {
  ...
  const fetchClientMaterials = async (clientId: string) => {
    ...
    try {
      const res = await fetch(`/api/clients/${clientId}/materials`);
      const data = await res.json();
      setClientMaterials(data || []);
    } catch (error) {
      console.error("Failed to fetch client materials:", error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentOrder && formData.order_items.length === 0) return toast.error("At least one inventory item must be deducted");
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const data = await res.json();
      if (data.error) toast.error("Delete failed");
      else {
