...
export function InlineMobileSearch({
  ...
  // SSR guard — only render portals after mount
  useEffect(() => {
    setPortalMounted(true);
  }, []);

  // Body scroll lock — mobile-safe position:fixed lock to block touch-drag scrolling on mobile
  useEffect(() => {
    if (!isActive) return;
  ...
  // ── Overlay content (scrim + dropdown) — portaled to document.body ──
  // This escapes the overflow-x-clip ancestor and any transform-bearing
  // wrappers (motion.div in PageTransition, sticky header, etc.) that
  // would trap position:fixed elements.
  const overlayContent = (
    <>
      ...
      <motion.div
        ...
        onClick={handleClose}
        aria-hidden="true"
        className="fixed inset-0 top-[44px] z-[100] bg-black/35 backdrop-blur-[1px] md:hidden cursor-pointer"
      />

      {/* ── Suggestion Panel Dropdown (fixed below header) ── */}
      <motion.div
        initial={{ opacity: 0, maxHeight: 0, y: -6 }}
        animate={{ opacity: 1, maxHeight: "calc(80dvh - 44px)", y: 0 }}
        exit={{ opacity: 0, maxHeight: 0, y: -6 }}
        transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
        className="fixed top-[44px] left-0 right-0 z-[101] overflow-hidden bg-background/95 backdrop-blur-md border-b border-x border-border/50 shadow-2xl rounded-b-2xl md:hidden"
      >
        <div className="max-h-[calc(75dvh-44px)] overflow-y-auto p-3 space-y-4 scrollbar-thin">
  ...
  return (
    <>
      {/* ── Inline Header Morph Search Bar Container (stays in header flow) ── */}
      <motion.div
      ...
      {/* ── Portaled overlay (scrim + dropdown) — escapes transform/overflow ancestors ── */}
      {portalMounted && createPortal(
        <AnimatePresence>{isActive && overlayContent}</AnimatePresence>,
        document.body
      )}
    </>
  );
